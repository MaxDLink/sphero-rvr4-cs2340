#fix not using RPi pin error by typing these into terminal 
...
#TODO - change these pins to pi4 pins 
right_trigger = 25 #pin 22 
right_echo = 12 #pin 32 
left_trigger = 23 #pin 16 
left_echo = 24 #pin 18 
...
async def main():
    ...
    while True:
        ...
        if dist_r < 35:
            while dist_r < 35:
                await rvr.raw_motors(2,128,1,128) #128 -->255
                dist_r =  distance_right()
                await asyncio.sleep(.05)
                print('turning right')
            await rvr.reset_yaw()
            ...
            while dist_l < 35:
                await rvr.raw_motors(1,128,2,128)  #128 -->255
                dist_l =   distance_left()
                await asyncio.sleep(.05)
                print('turning left')
            await rvr.reset_yaw()
        elif dist_l >= 35 and dist_r >= 35:
            await rvr.drive_with_heading(50,0,0) #forward speed 
